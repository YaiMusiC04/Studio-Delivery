// Two modes:
//   1. imageBase64 provided → Claude Vision extracts positions from photo
//   2. positions[] + goal string → Claude generates a personalized investment plan
  ...
  try {
    ...
    return positions.map(p => {
      ...
      return {
        ...
        changeToday: q.regularMarketChangePercent,
      }
    })
...
Incluye TODAS las posiciones visibles. Si un valor no aparece, usa null.` },
    ...
    // Refresh prices from Yahoo Finance for accuracy
...
Analiza si la meta es alcanzable y genera un plan. Devuelve este JSON exacto:
{
  ...
  "recommendedAllocation": [
    { "type": "Nombre (ej: ETFs de índice)", "percentage": number, "examples": ["VTI","QQQ"], "reason": "Por qué este tipo" }
  ],
  "milestones": [
    { "year": 2027, "expectedValue": number, "cumulativeReturnPct": number }
  ],
  "topPicks": [
    { "symbol": "TICKER", "name": "Nombre", "why": "Por qué para esta meta", "suggestedAllocationPct": number }
  ],
  "risks": ["Riesgo inflación", "Riesgo mercado"],
  "actionSteps": ["Paso concreto 1", "Paso concreto 2", "Paso concreto 3"]
}`,
      2200
    )
    plan = parseJson(text)
